...
Toolbar_Add(hGui, Handler, Style="", ImageList="", Pos="") {
    ...
    , TBSTYLE_WRAPABLE := 0x200, TBSTYLE_FLAT := 0x800, TBSTYLE_LIST := 0x1000, TBSTYLE_TOOLTIPS := 0x100, TBSTYLE_TRANSPARENT := 0x8000, TBSTYLE_ADJUSTABLE := 0x20
	, TBSTYLE_VERTICAL := 0x80, TBSTYLE_EX_DRAWDDARROWS := 0x1, TBSTYLE_EX_HIDECLIPPEDBUTTONS := 0x10, TBSTYLE_EX_MIXEDBUTTONS := 0x8
	, TB_BUTTONSTRUCTSIZE := 0x41E, TB_SETEXTENDEDSTYLE := 0x454, TB_SETUNICODEFORMAT := 0x2005
	, TBSTYLE_NODIVIDER := 0x40, CCS_NOPARENTALIGN := 0x8, CCS_NORESIZE := 0x4, TBSTYLE_BOTTOM := 0x3, TBSTYLE_MENU := 0, TBSTYLE_BORDER := 0x800000
	...
	LoopParse, Style, %A_Tab%%A_Space%, %A_Tab%%A_Space%
		if (!A_LoopField)
			continue
	...
	if (Pos != ""){
		x := y := 0, w := h := 100
		LoopParse, Pos, %A_Tab%%A_Space%, %A_Tab%%A_Space%
		{
			if (!A_LoopField)
				continue
...
Toolbar_GetRect(hCtrl, Pos="", pQ="") {
	...
	if pPos !=
		if (Pos <= 0)
			return "Err: Invalid button position"

	VarSetCapacity(RECT, 16)
    SendMessage, TB_GETITEMRECT, Pos-1,&RECT, ,ahk_id %hCtrl%
	If (ErrorLevel = 0)
		return A_ThisFunc "> Can't get rect"
...
Toolbar_Insert(hCtrl, Btns, Pos=""){
	static TB_ADDBUTTONSW := 0x444, TB_INSERTBUTTONW := 0x443

	cnt := Toolbar_compileButtons(hCtrl, Btns, cBTN)
	if (Pos = "")
		SendMessage, TB_ADDBUTTONSW, cnt, cBTN ,, ahk_id %hCtrl%

	else loop cnt
		SendMessage, TB_INSERTBUTTONW, Pos+A_Index-2, cBTN + 20*(A_Index-1) ,, ahk_id %hCtrl%
...
Toolbar_SetButton(hCtrl, WhichButton, State="", Width=""){
	...
	, TB_SETBUTTONINFO := 0x442, TB_GETSTATE := 0x412, TB_GETBUTTON := 0x417
	, TBSTATE_CHECKED := 1, TBSTATE_ENABLED := 4, TBSTATE_HIDDEN := 8, TBSTATE_ELLIPSES := 0x40, TBSTATE_DISABLED := 0
	...
	if InStr(State, "-disabled") {
		...
		StrReplace, State, State, -disabled
	}
	else if InStr(State, "disabled")
		hState &= ~TBSTATE_ENABLED

	LoopParse, State, %A_Tab%%A_Space%, %A_Tab%%A_Space%
	{
		if (!A_LoopField)
			continue
	...
	if (!hState)
		return A_ThisFunc "> Some of the states are invalid: " State
...
Toolbar_ToggleStyle(hCtrl, Style="LIST"){
	...
	, TB_SETSTYLE := 1080, TB_GETSTYLE := 1081

	s := Style+0 != "" ? Style : TBSTYLE_%Style%	
	if (!s)
		return A_ThisFunc "> Invalid style: " Style
...
Toolbar_compileButtons(hCtrl, Btns, ByRef cBTN) {
	...
	, TBSTATE_CHECKED := 1, TBSTATE_ENABLED := 4, TBSTATE_HIDDEN := 8, TBSTATE_DISABLED := 0, TBSTATE_WRAP := 0x20
	, TB_ADDSTRINGW := 0x44D, WS_CLIPSIBLINGS := 0x4000000
	, id := 10000								;automatic IDing starts form 10000,     1 <= userID < 10 000
	...
	LoopParse, Btns, `n, %A_Space%%A_Tab%	
	{
	 ...
	 ;parse states
		...
		LoopParse, a3, %A_Tab%%A_Space%, %A_Tab%%A_Space%
		{
			if (!A_LoopField)
				continue
			hState |= TBSTATE_%A_LOOPFIELD%
		...
		if (!hState)
			return A_ThisFunc "> Some of the states are invalid: " a3
	 ...
	 ;parse styles
		...
		LoopParse, a4, %A_Tab%%A_Space%, %A_Tab%%A_Space%
		{
			if (!A_LoopField)
				continue
		...
		if (!hStyle)
			return A_ThisFunc "> Some of the styles are invalid: " a4
	 ...
	 ;add caption to the string pool
		if (hStyle != BTNS_SEP) {
			StrReplace a1, a1, `r, `n, A		;replace `r with new lines (for multiline tooltips)
			VarSetCapacity(buf, StrLen(a1)*2+2, 0), buf := a1	 ;Buf must be double-NULL-terminated, use unicode length in both cases.
			sIdx := DllCall("SendMessage","uint",hCtrl,"uint", TB_ADDSTRINGW, "uint", 0, "str", buf)  ;returns the new index of the string within the string pool
...
Toolbar_onNotify(Wparam,Lparam,Msg,Hwnd) { 
	...
	, TBN_BEGINADJUST := -703, TBN_GETBUTTONINFOA := -700, TBN_GETBUTTONINFOAW := -720, TBN_QUERYINSERT := -706, TBN_QUERYDELETE := -707
	, TBN_ENDADJUST := -704, TBN_RESET := -705, TBN_TOOLBARCHANGE := -708, TB_COMMANDTOINDEX := 0x419
	, cnt, cnta, cBTN, inDialog, tc := 0
...
Toolbar_getStateName( hState ) {
	...
	, states := "hidden,pressed,checked,enabled"
	...
	StrReplace state, state, %A_Space%enabled%A_Space%
	return state
}
...
Toolbar_getStyleName( hStyle ) {
	...
	, styles := "check,group,dropdown,autosize,noprefix,showtext"
	
	LoopParse, styles, `,
		if (hStyle & BTNS_%A_LoopField%)
			style .= A_LoopField " "
	StrReplace, style, style, check group, checkgroup		;I don't use group flag at all
	return style
}
