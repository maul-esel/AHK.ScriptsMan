...
Panel_Add(HParent, X="", Y="", W="", H="", Style="", Text="") {
	...
	If (hCtrl = 0)
		return A_ThisFunc "> Failed to create control."
	return hCtrl
} 
...
Panel_SetStyle(Hwnd, Style, ByRef hStyle="", ByRef hExStyle="") {
	...
	;set custom styles
		if j := InStr(pStyle, "scroll")
		{
			...
			if (c = "h")
				r := DllCall("SetWindowLong", "uint", Hwnd, "int", GWL_USERDATA, "Uint", 1)
			if (c = "v")
				r := DllCall("SetWindowLong", "uint", Hwnd, "int", GWL_USERDATA, "Uint", 2)
			if (c = A_Space)
				r := DllCall("SetWindowLong", "uint", Hwnd, "int", GWL_USERDATA, "Uint", 3)
...
Panel_wndProc(Hwnd, UMsg, WParam, LParam) { 
	...
	static	WM_SIZE := 5, WM_SHOWWINDOW := 24, WM_CHANGEUISTATE := 295, GWL_ROOT := 2, GWL_WNDPROC := -4, GWL_USERDATA := -2
			...
			, redirect := "78,273,0x204,0x205"  ;WM_COMMAND=78, WM_NOTIFY=273, WM_RBUTTONDOWN=0x204, WM_RBUTTONUP=0x205
...
;		return DllCall("SendMessage", "uint", hRoot, "uint", UMsg, "uint", WParam, "uint", LParam)  ;redirection type 2	- use root hwnd
	...
	if UMsg in 276,277		
		if (LParam != 0)
			return DllCall(rootProc, "uint", Hwnd, "uint", UMsg, "uint", WParam, "uint", LParam)	 ;scrollbar control.
